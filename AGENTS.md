...
`devlog/` is ordinary tracked documentation, not a private mirror or submodule:

- `devlog/_plan/` — open units and planning evidence.
- `devlog/_fin/` — closed units whose outcome is already public.
- `devlog/_chase/` — gitignored third-party reference material.

The runtime does not read `devlog/`; `privacy:scan` deliberately does. Keep
repository hygiene intact: tracked gitlinks, vendored reference clones, and
previously excised security triage must not return to the index.
...
Keep consent explicit in the operating instructions and in code paths that
perform such actions. New identity-spending actions need the same gate and a
focused regression test; do not rely on an agent prompt that can answer itself.
...
runtime, routing, configuration, or server behavior is affected, a focused
check fails or is ambiguous, the user asks for full validation, or a non-trivial
PR is being marked review-ready. Do not rerun passing checks on unchanged code.
