...
const commandRunners: Record<string, CommandRunner> = {
  ...
  "split-bridge": async deps => {
    const action = deps.args[1] ?? "status";
    const plistDigestArg = deps.args[2];
    const validStartDigest = action === "start"
      && deps.args.length === 3
      && /^--split-plist-digest=[a-f0-9]{64}$/.test(plistDigestArg ?? "");
    if ((!validStartDigest && deps.args.length > 2)
      || !["install", "load", "status", "start", "stop", "uninstall", "remove", "repair"].includes(action)) {
